...
Describe "New-PingRequestObject" -Tag 'function','private' {
    ...
    It 'should check if input for NamespaceURI is null or whitespace, and if so throw an error' {
        {New-PingRequestObject -NamespaceURI ' ' -NamespaceSchema 'validinput' -ErrorAction Stop} | Should -Throw
    }
    It 'should check if input for NamespaceSchema is null or whitespace, and if so throw an error' {
        {New-PingRequestObject -NamespaceURI 'validinput' -NamespaceSchema ' ' -ErrorAction Stop} | Should -Throw
    }
}